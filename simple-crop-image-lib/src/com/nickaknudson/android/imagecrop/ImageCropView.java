...
public class ImageCropView extends ImageViewTouchBase {
	...
	ImageCropViewDelegate delegate;
	 ...
	 *
	 */
	public interface ImageCropViewDelegate {
		/**
		 * @return saving
		 */
		public boolean isSaving();
		/**
		 * @return waiting to pick
		 */
	 ...
	 * @param delegate
	 */
	public void setDelegate(ImageCropViewDelegate delegate) {
		this.delegate = delegate;
	}

	@Override
	public boolean onTouchEvent(MotionEvent event) {
		// if the delegate hasn't been setup, then just return
		if (delegate == null) {
			return false;
		}
		// if we are saving, we don't want to make changes
		if (delegate.isSaving()) {
			return false;
		}
		// otherwise adjust accordingly
		switch (event.getAction()) {
		case MotionEvent.ACTION_DOWN:
			if (delegate.isWaitingToPick()) {
			...
			if (delegate.isWaitingToPick()) {
				for (int i = 0; i < mHighlightViews.size(); i++) {
					...
					if (hv.hasFocus()) {
						...
						delegate.setWaitingToPick(false);
						return true;
					}
				}
